...
$id = $_GET['id'];
    ...
    $statCheck = $conn->prepare("SELECT Hide_stats FROM users WHERE id = ?");
    $statCheck->bind_param("i", $id);
    $statCheck->execute();
    $statResult = $statCheck->get_result();

    if ($statResult->num_rows > 0) {
        $publicStats = (int)$statResult->fetch_assoc()["Hide_stats"];
        if ($publicStats === 1) {
            // Stats are hidden, exit without returning any data
            echo json_encode([
                "success" => false,
                "message" => "User statistics are hidden"
            ]);
            exit();
        }
    } else {
